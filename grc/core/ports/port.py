...
class Port(Element):
    ...
    def __init__(
        ...
        **_
        ...
        if not label:
            label = (
                id if not id.isdigit() else {"sink": "in", "source": "out"}[direction]
            )
        if dtype == "bus":
            # Look for existing busses to give proper index
            busses = [
                p
                for p in self.parent.ports()
                if p._dir == self._dir and p.dtype == "bus"
            ]
    ...
    def __repr__(self):
        return "{!r}.{}[{}]".format(
            self.parent, "sinks" if self.is_sink else "sources", self.key
        )
    ...
    def validate(self):
        ...
        try:
            ...
            if self.is_sink and not domain.multi_in and num_connections > 1:
                self.add_error_message(
                    'Domain "{}" can have only one upstream block'
                    "".format(self.domain)
                )
            if self.is_source and not domain.multi_out and num_connections > 1:
                self.add_error_message(
                    'Domain "{}" can have only one downstream block'
                    "".format(self.domain)
                )
        except KeyError:
            self.add_error_message(
    ...
    def resolve_empty_type(self):
        ...
        try:
            port = find_port(_virtual_connections.upstream_ports) or find_port(
                _virtual_connections.downstream_ports
            )
            self.set_evaluated(
                "dtype", port.dtype
    ...
    def connections(self, enabled=None):
        ...
        for con in self.parent_flowgraph.connections:
            ...
            if self.dtype == "bus":
                if self.is_sink:
                    if (
                        self.parent.name == con.sink_port.parent.name
                        and self.name == con.sink_port.name
                    ):
                        port_in_con = True
                elif self.is_source:
                    if (
                        self.parent.name == con.source_port.parent.name
                        and self.name == con.source_port.name
                    ):
                        port_in_con = True
    ...
    def get_associated_ports(self):
        if not self.dtype == "bus":
            ...
            if bus_structure:
                ...
                ports = filter(
                    lambda a: ports.index(a) in bus_structure[bus_index], ports
                )
            return ports
