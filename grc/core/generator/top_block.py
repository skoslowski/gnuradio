...
class TopBlockGenerator(object):
    ...
    def write(self):
        ...
        fg = self._flow_graph
        self.title = (
            fg.get_option("title") or fg.get_option("id").replace("_", " ").title()
        )
    ...
    def _build_python_code_from_template(self):
        ...
        flow_graph_code = "\n".join(
            line.rstrip() for line in flow_graph_code.split("\n")
        )
    ...
    def _imports(self):
        ...
        need_path_hack = any(
            imp.endswith("# grc-generated hier_block") for imp in imports
        )
        if need_path_hack:
            output.insert(
                0,
    ...
    def _blocks(self):
        ...
        def _get_block_sort_text(block):
            ...
            try:
                code += block.params[
                    "gui_hint"
                ].get_value()  # Newer gui markup w/ qtgui
            except KeyError:
                # No gui hint
                pass
            return code
    ...
    def _callbacks(self):
        ...
        callbacks_all = []
        for block in fg.iter_enabled_blocks():
            callbacks_all.extend(
                expr_utils.expr_replace(cb, replace_dict)
                for cb in block.get_callbacks()
            )
        ...
        def uses_var_id(callback):
            ...
            return used and (
                ("self." + var_id in callback) or ("this->" + var_id in callback)
    ...
    def _connections(self):
        ...
        virtual_source_connections = [
            c for c in connections if isinstance(c.source_block, blocks.VirtualSource)
        ]
        ...
        virtual_connections = [
            ...
            if (
                isinstance(c.source_block, blocks.VirtualSource)
            ...
            if con.source_port.dtype != "bus":
                ...
                if porta.dtype == "bus" and portb.dtype == "bus":
                    ...
                    if len(porta.bus_structure) == len(portb.bus_structure):
                        for port_num_a, port_num_b in zip(
                            porta.bus_structure, portb.bus_structure
                        ):
