...
class HierBlockGenerator(TopBlockGenerator):
    ...
    def __init__(self, flow_graph, file_path):
        ...
        self.file_path = os.path.join(
            hier_block_lib_dir, self._flow_graph.get_option("id") + ".py"
        )
    ...
    def _build_block_n_from_flow_graph_io(self):
        ...
        data["id"] = block_id
        data["label"] = (
            self._flow_graph.get_option("title")
            or self._flow_graph.get_option("id").replace("_", " ").title()
        )
        ...
        # Callback data
        t["callbacks"] = [
            "set_{key}(${{ {key} }})".format(key=param_block.name)
            for param_block in parameters
        ]
