...
class FlowGraph(Element):
    ...
    def __str__(self):
        return "FlowGraph - {}({})".format(
            self.get_option("title"), self.get_option("id")
        )
    ...
    def renew_namespace(self):
        ...
        for expr in self.imports():
            try:
                ...
                log.exception(
                    'Failed to evaluate import expression "{0}"'.format(expr),
                    exc_info=True,
                )
                pass
        ...
        for id, expr in self.get_python_modules():
            try:
                ...
                log.exception(
                    "Failed to evaluate expression in module {0}".format(id),
                    exc_info=True,
                )
                pass
        ...
        for parameter_block in self.get_parameters():
            try:
                ...
                log.exception(
                    "Failed to evaluate parameter block {0}".format(
        ...
        for variable_block in self.get_variables():
            try:
                ...
                log.exception(
                    "Failed to evaluate variable block {0}".format(variable_block.name),
                    exc_info=True,
                )
                pass
    ...
    def import_data(self, data):
        ...
        try:
            ...
            for src_blk_id, src_port_id, snk_blk_id, snk_port_id in data.get(
                "connections", []
            ):
