...
class Param(Element):
    ...
    def __init__(
        ...
        **_
        ...
        self.options = self._init_options(
            options or [], option_labels or [], option_attributes or {}
        )
    ...
    def _init_options(self, values, labels, attributes):
        ...
        for i, option in enumerate(values):
            ...
            options.attributes[option] = {
                attrib: values[i] for attrib, values in six.iteritems(attributes)
            }
    ...
    def evaluate(self):
        ...
        #########################
            ...
            return (
                self.parse_gui_hint(expr)
                if self.parent_block.state == "enabled"
                else ""
            )
        #########################
        # Import Type
    ...
    def parse_gui_hint(self, expr):
        ...
        def parse_pos():
            ...
            if (
                ...
                or len(e) not in (2, 4)
                or not all(isinstance(ei, int) for ei in e)
            ):
                raise Exception(
                    "Invalid GUI Hint entered: {e!r} (Must be a list of {{2,4}} non-negative integers).".format(
                        e=e
                    )
                )
            ...
            if (row < 0) or (col < 0):
                raise Exception(
                    "Invalid GUI Hint entered: {e!r} (non-negative integers only).".format(
                        e=e
                    )
                )
            ...
            if (row_span < 1) or (col_span < 1):
                raise Exception(
                    "Invalid GUI Hint entered: {e!r} (positive row/column span required).".format(
                        e=e
                    )
                )
        ...
        def validate_tab():
            ...
            if not tab_block:
                raise Exception(
                    "Invalid tab name entered: {tab} (Tab name not found).".format(
                        tab=tab
                    )
                )
            ...
            if index >= tab_index_size:
                raise Exception(
                    "Invalid tab index entered: {tab}@{index} (Index out of range).".format(
                        tab=tab, index=index
                    )
                )
