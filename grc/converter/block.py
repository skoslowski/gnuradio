...
def convert_block_xml(node):
    converter = cheetah_converter.Converter(
        names={
            param_node.findtext("key"): {
                ...
                for opt_node in next(
                    param_node.iterfind("option"), param_node
                ).iterfind("opt")
            }
    ...
    data["parameters"] = [
        convert_param_xml(param_node, converter.to_python_dec)
        for param_node in node.iterfind("param")
    ] or no_value
    # data['params'] = {p.pop('key'): p for p in data['params']}

    data["inputs"] = [
        convert_port_xml(port_node, converter.to_python_dec)
        for port_node in node.iterfind("sink")
    ] or no_value

    data["outputs"] = [
        convert_port_xml(port_node, converter.to_python_dec)
        for port_node in node.iterfind("source")
    ] or no_value
    ...
    data["asserts"] = [
        converter.to_python_dec(check_node.text)
        for check_node in node.iterfind("check")
    ] or no_value
    ...
    data = OrderedDict(
        (key, value) for key, value in data.items() if value is not no_value
    )
...
def auto_hide_params_for_item_sizes(data):
    ...
    vlen_templates = []
    for port in chain(
        *[data.get(direction, []) for direction in ["inputs", "outputs"]]
    ):
...
def convert_templates(node, convert, block_id=""):
    ...
    imports = "\n".join(
        convert(import_node.text) for import_node in node.iterfind("import")
    )
    if "\n" in imports:
    ...
    return OrderedDict(
        (key, value) for key, value in templates.items() if value is not no_value
    )
...
def convert_param_xml(node, convert):
    ...
    option_labels = yaml.ListFlowing(
        on.findtext("name") for on in node.iterfind("option")
    )
    ...
    return OrderedDict(
        (key, value) for key, value in param.items() if value is not no_value
    )
...
def convert_port_xml(node, convert):
    ...
    if domain == "message":
        ...
        port["vlen"] = (
            int(vlen) if vlen and vlen.isdigit() else convert(vlen) or no_value
        )
    ...
    return OrderedDict(
        (key, value) for key, value in port.items() if value is not no_value
    )
