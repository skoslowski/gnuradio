...
class Converter(object):
    ...
    def convert_hard(self, expr, spec=Python):
        lines = "\n".join(
            self.convert_hard_line(line, spec) for line in expr.split("\n")
        )
        if spec == Mako:
    ...
    def convert_hard_replace(self, expr, spec=Python):
        ...
        while pos < len(expr):
            ...
            if char == "$":
                ...
                delim_to_find and char in delim_to_find and all_delims_closed()
            ):  # end of substitution
                out.append(spec.end)
                if char in delims[""]:
                    out.append(char)
                ...
                delim_to_find and char in ")]}" and extra_close()
            ):  # end of substitution
                out.append(spec.end)
                out.append(char)
                delim_to_find = False
