...
class Action(Gio.SimpleAction):
    ...
    def __init__(
        ...
        if keypresses:
            self.keypresses = [
                kp.replace("<Ctrl>", Utils.get_modifier_key(True)) for kp in keypresses
            ]
        else:
...
ABOUT_WINDOW_DISPLAY = actions.register(
    "app.about", label="_About", tooltip="About this program", icon_name="help-about",
)
...
PORT_CONTROLLER_DEC = actions.register(
    "win.port_controller_dec", keypresses=["KP_Subtract", "minus"],
)
PORT_CONTROLLER_INC = actions.register(
    "win.port_controller_inc", keypresses=["KP_Add", "plus"],
)
