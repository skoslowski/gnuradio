...
def test_simple_format_string():
    convert = functools.partial(
        parser.Converter(names={"abc"}).convert_simple, spec=parser.FormatString
    )
    assert "{abc}" == convert("$abc")
...
def test_hard_format_string():
    ...
    convert = functools.partial(
        parser.Converter(names).convert_hard, spec=parser.FormatString
    )
    assert "make_a_cool_block_{abc.ff}({param1}, {param2})" == convert(
        "make_a_cool_block_${abc.ff}($param1, $param2)"
    )
